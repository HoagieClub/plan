...
def check_user(user_inst: CustomUser, major, minors, certificates):
	...
	if major is not None:
		...
		for degree in degrees:
			...
			formatted_req = check_requirements(
				user_inst, "Degree", degree.code, copy.deepcopy(user_courses), manually_satisfied_reqs
			)
		...
		if major_code != "Undeclared":
			formatted_req = check_requirements(
				user_inst, "Major", major_code, copy.deepcopy(user_courses), manually_satisfied_reqs
			)
		else:
	...
	for minor in minors:
		...
		formatted_req = check_requirements(
			user_inst, "Minor", minor_code, copy.deepcopy(user_courses), manually_satisfied_reqs
		)
	...
	for certificate in certificates:
		...
		formatted_req = check_requirements(
			user_inst, "Certificate", certificate_code, copy.deepcopy(user_courses), manually_satisfied_reqs
		)
...
def create_courses(user_inst):
	...
	course_insts = (
		UserCourses.objects.select_related("course", "course__department")
...
def cleanup_invalid_usercourses():
	...
	invalid_records = UserCourses.objects.filter(Q(course__isnull=True) | Q(course__department__isnull=True))
