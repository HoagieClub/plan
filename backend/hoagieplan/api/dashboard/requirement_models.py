class UserRequirements:
	def __init__(self, data: dict):
		...
		if "Minors" in data:
			self.minors = {minor: Requirement(value["requirements"]) for minor, value in data["Minors"].items()}
			keys.remove("Minors")
		else:
...
class Requirement:
	...
	def __str__(self):
		...
		if self.subrequirements is not None:
			fields["subrequirements"] = {key: str(value) for key, value in self.subrequirements.items()}
...
class CourseList:
	...
	def __str__(self):
		fields = {"course_list_id": self.course_list_id, "courses": [str(course) for course in self.courses]}
		return str(fields)
