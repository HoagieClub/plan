...
DAYS_OFFSET = {'M': timedelta(days = 0), 'T': timedelta(days = 1),
               'W': timedelta(days = 2), 'Th': timedelta(days = 3),
               'F': timedelta(days = 4)}
...
def generate_class_ical(cal, calendar_event, semester_code):
    ...
    if not all([course_name, section_number, start_time, 
                end_time, days_of_week, start_date, end_date]):
