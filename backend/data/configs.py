...
class Configs:
	...
	def get_base_url(self, endpoint):
		return ACTIVE_DIRECTORY_BASE_URL if "users/" in endpoint else STUDENT_APP_BASE_URL
	...
	def _refreshToken(self, **kwargs):
		...
		with requests.post(self.REFRESH_TOKEN_URL, data=kwargs, headers=headers) as resp:
			response = resp.json()
