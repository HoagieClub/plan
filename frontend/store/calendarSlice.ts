...
import type { Profile, CalendarEvent, ClassMeeting, Course, Section } from '@/types';
...
const useCalendarStore = create<CalendarStore>()((set, get) => ({
	...
	addCourse: async (course: Course) => {
		...
		try {
			...
			// if no calendar for term, create one
			const calExists = await calendarService.hasCalendarForTermInDB(termInt);
			const calendarName = `${profile.firstName}'s Calendar ${term}`;
			if (!calExists) {
